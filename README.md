...
| Back‑end  | **Python 3.10**, **Flask 3** (blueprints), **SQLAlchemy 2** ORM, **Flask‑JWT‑Extended** auth.  |
...
│   ├── routes/               # Blueprint packages (auth, flights, bookings…)
...
| `ORACLE_PW`      | `secret_pw`               | Password for the above user.              |
| `JWT_SECRET_KEY` | `🔑`                      | 256‑bit secret for signing access tokens. |
...
$ export FLASK_APP=backend.app:auto_app
...
```bash
$ gunicorn 'backend.app:auto_app()' -b 0.0.0.0:8000 --workers 4 --timeout 120
```
...
* Rate limiting (Flask‑Limiter) on login & search endpoints.
